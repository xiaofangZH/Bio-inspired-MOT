...
class DINOv3Wrapper(nn.Module):
    ...
    def __init__(self, output_dim=256, weights_path=None, use_checkpoint=True, max_blocks=None):
        ...
        if hasattr(self.vit, 'get_intermediate_layers') or hasattr(self.vit, 'prepare_tokens_with_masks'):
            self._vit_type = 'dinov3'
        elif hasattr(self.vit, 'prepare_tokens'):
            self._vit_type = 'dinov3'
        elif hasattr(self.vit, 'forward_features'):
            self._vit_type = 'timm'
        else:
    ...
    def get_frozen_status(self):
        ...
        frozen_params = sum(1 for p in self.vit.parameters()
                            if not p.requires_grad)
        total_params = sum(1 for p in self.vit.parameters())
        return {"frozen": frozen_params, "total": total_params, "ratio": frozen_params / total_params}

    # ===== 冻结/解冻方法 (三步走训练使用) =====

    def freeze_all_backbone(self):
        """兼容接口：当前训练策略固定为全参数训练。"""
        self.unfreeze_all_except_patch_embed()

    def unfreeze_top_blocks(self, n=8):
        """兼容接口：当前训练策略固定为全参数训练。"""
        self.unfreeze_all_except_patch_embed()

    def unfreeze_all_except_patch_embed(self):
        """启用全参数训练：backbone/proj/norm 全部可训练。"""
        for param in self.vit.parameters():
            param.requires_grad = True
        for param in self.proj.parameters():
            param.requires_grad = True
