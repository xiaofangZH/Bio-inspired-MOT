...
from data_loader import create_dataloader, load_frame_tensor
...
class Config:
    ...
    def __init__(
        ...
        self.dataset_paths = {
            "dancetrack": {"train": yd.get("dancetrack", {}).get("train", "hmat/data/OpenDataLab___DanceTrack/raw/train"),
                           "val": yd.get("dancetrack", {}).get("val", "hmat/data/OpenDataLab___DanceTrack/raw/val")},
...
class Trainer:
    def __init__(self, config: Config):
        ...
        self.criterion = SetCriterion(
            ...
            label_smoothing=0.0,  # Focal Loss 与 Label Smoothing 冲突：背景噪声淹没回归梯度
        )
        ...
        self.logger.info("模型总参数: %s (预冻结，阶段配置后将缩减)", f"{total:,}")
    ...
    def _restore_optimizer_scheduler(self, resume_info: dict, stage_cfg: dict, n_epochs: int, remaining_epochs: int):
        """从 resume_info 恢复 optimizer 和 scheduler 状态。

        Args:
            resume_info: 包含 optimizer_state_dict 和 scheduler_state_dict
            stage_cfg: 当前阶段配置
            n_epochs: 当前阶段的 总 epoch 数（用于 CosineAnnealingLR 的 T_max）
            remaining_epochs: 剩余要训练的 epoch 数
        """
        # ─── Bug1修复：使用 get_parameter_groups 保持分层参数组结构 ───
        # 必须与 Trainer.__init__ 中创建优化器的方式完全一致，
        # 否则 load_state_dict 会因 param_groups 数量不匹配而失败，
        # 导致 Adam 动量状态 (exp_avg, exp_avg_sq) 全部丢失。
        backbone_lr_scale = self.config.backbone_base_lr / self.config.head_base_lr
        param_groups = self.model.get_parameter_groups(
            base_lr=self.config.head_base_lr,
        ...
        # 尝试加载 optimizer 状态（恢复 Adam 动量）
        opt_state = resume_info.get("optimizer_state_dict")
        if opt_state:
            try:
                self.optimizer.load_state_dict(opt_state)
                loaded_lrs = [pg["lr"] for pg in self.optimizer.param_groups]
                self.logger.info("✓ Optimizer 状态已恢复 (含Adam动量), param_groups=%d, LRs=%s",
                               len(self.optimizer.param_groups), [f"{lr:.6e}" for lr in loaded_lrs])
            except Exception as e:
                self.logger.warning("Optimizer 状态恢复失败: %s，使用配置学习率初始化", e)

        # ─── Bug3修复：T_max 固定为阶段总 epoch 数 n_epochs ───
        # CosineAnnealingLR 的衰减周期由 T_max 决定。
        # 必须始终使用总轮数 n_epochs（而非 remaining_epochs），
        # 然后通过 load_state_dict 设置 last_epoch 定位到正确的衰减点，
        # 否则余弦波形的频率会失真，恢复后学习率产生突变断层。
        self.scheduler = optim.lr_scheduler.CosineAnnealingLR(
            self.optimizer,
            T_max=n_epochs,
            eta_min=0,
        ...
        current_lrs = [f"{pg['lr']:.6e}" for pg in self.optimizer.param_groups]
        self.logger.info(
            "恢复训练: %d trainable params %s, remaining_epochs=%d, T_max=%d, LRs=%s",
            trainable, freeze_str, remaining_epochs, n_epochs, current_lrs,
        )
    ...
    def _build_targets(self, frames):
        ...
        for frame in frames:
            ...
            if boxes:
                ...
                track_ids_t = torch.tensor(track_ids, dtype=torch.long, device=self.config.device)
            else:
                ...
                track_ids_t = torch.zeros((0,), dtype=torch.long, device=self.config.device)

            total_gt += boxes_t.shape[0]
            targets.append({"boxes": boxes_t, "labels": labels_t, "track_ids": track_ids_t})
    ...
    def _compute_loss_and_metrics(self, outputs, targets):
        ...
        # NaN/Inf 检测: 检查所有输出张量（不再依赖 _sanitize_outputs 掩盖 NaN）
        for k, v in outputs.items():
            if isinstance(v, torch.Tensor) and v.numel() > 0:
                if not torch.isfinite(v).all():
                    self.logger.warning(
                        "模型输出含 NaN/Inf (key=%s): shape=%s, range=[%.4f, %.4f], "
                        "nan_count=%d, inf_count=%d",
                        k, list(v.shape),
                        float(v[torch.isfinite(v)].min()) if torch.isfinite(v).any() else float('nan'),
                        float(v[torch.isfinite(v)].max()) if torch.isfinite(v).any() else float('nan'),
                        int((~torch.isfinite(v)).sum().item()),
                        int(torch.isinf(v).sum().item()) if not torch.isnan(v).all() else 0,
                    )
                    # 一旦发现 NaN，立即跳过该步
                    return None, {}
        ...
        # 应用 YAML 配置中的 Loss 权重（criterion.forward 返回原始 Loss）
        ...
        # 额外计算指标（使用 SetCriterion 返回的两阶段匹配 indices）
        with torch.no_grad():
    ...
    def save_checkpoint(self, epoch_index: int, step_tag: Optional[int] = None):
        """Save checkpoint. 流式 offload 到 CPU，避免 GPU RAM 峰值。

        关键优化：
        - 不用 state_dict()（该调用会一次性将全部 ~97M 参数的 GPU 引用装入 dict，
          额外占用 ~388 MB GPU 显存，叠加模型自身、梯度、优化器状态后频繁触发 CUDA OOM）。
        - 改为逐一遍历 named_parameters/named_buffers，每次仅持有一个 GPU 张量引用，
          offload 后立即丢弃。
        - torch.save 前后强制 gc.collect + empty_cache。
        """
        import torch
        import gc
        ...
        # ─── 逐参数 offload（不通过 state_dict 全量抓取）───
        model_sd = {}
        for name, param in self.model.named_parameters():
        ...
        opt_sd = {}
        opt_raw = self.optimizer.state_dict()
    ...
    def train(self, train_dataloader, val_dataloader=None, stage_config: Optional[dict] = None, global_epoch_offset: int = 0, step_in_stage_start: int = 0):
        ...
        for epoch in range(self.config.epochs):
            ...
            nan_step_count = 0  # 追踪 NaN 步数

            for batch_idx, batch in enumerate(train_dataloader):
                sequences = batch if isinstance(batch, list) else [batch]
                for seq_idx, sequence in enumerate(sequences):
                    # ─── 延迟加载：从元数据获取路径，不加载图像 ───
                    img_paths = sequence.get("img_paths", [])
                    annotations = sequence.get("annotations", {})
                    img_size = sequence.get("img_size", self.config.img_size)
                    ...
                    for frame_idx, img_path in enumerate(img_paths):
                        ...
                        try:
                            ...
                            with torch.amp.autocast('cuda', dtype=torch.bfloat16, enabled=self.config.use_amp and "cuda" in self.config.device):
                                loss, metrics = self._forward_pass([frame_data])
                            if loss is None:
                                failed_steps += 1
                                nan_step_count += 1
                                if nan_step_count % 50 == 1:
                                    self.logger.warning(
                                        "NaN step count: %d (seq=%s frame=%s)",
                                        nan_step_count, seq_idx, frame_idx,
                                    )
                                self.logger.warning(
                                    "loss=None (step=%s, seq=%s, frame=%s, failed_total=%s)",
                                    self.global_step, seq_idx, frame_idx, failed_steps,
                                )
                                if failed_steps <= 3:
                                    self.logger.warning(
                            ...
                            # 先将 loss 除以累积步数，再通过 scaler 缩放回传
                            # 这样 scaler 的 loss_scale 检测基于平均值，不会因累积放大而误判溢出
                            ...
                            if accum_step % self.config.gradient_accumulation == 0:
                                # 每 accum_steps 步统一执行 optimizer.step
                                if self.scaler is not None:
                            ...
                            num_steps += 1
                            self.global_step += 1
                            processed_frames += metrics.get("frame_count", 1)
                            ...
                            if self.global_step % self.config.checkpoint_interval_steps == 0:
                                self.logger.info("保存检查点 (step=%s)...", self.global_step)
                                try:
                                    self.save_checkpoint(epoch, step_tag=self.global_step)
                                except Exception as _ckpt_exc:
                                    self.logger.error("检查点保存失败 (step=%s): %s", self.global_step, _ckpt_exc)
                                # checkpoint 保存后强制 GC，防止 pickle 缓冲累积
                                import gc as _gc
                                _gc.collect()
                                torch.cuda.empty_cache()
                            ...
                            # OOM / CUDA 错误：记录并跳过，清理显存
                            failed_steps += 1
                            err_msg = str(exc)
                            self.logger.warning(
                                "训练步失败 (CUDA, batch=%s seq=%s frame=%s): %s",
                                batch_idx, seq_idx, frame_idx, err_msg[:200],
                            )
                            torch.cuda.empty_cache()
                            continue
                            ...
                            failed_steps += 1
                            self.logger.warning(
                                "训练步数值错误 (batch=%s seq=%s frame=%s): %s",
                                batch_idx, seq_idx, frame_idx, exc,
                            )
                            continue
                        except (OSError, IOError) as exc:
                            # 文件读取/PIL 图像加载错误：记录并跳过
                            failed_steps += 1
                            self.logger.warning(
                                "训练步文件错误 (batch=%s seq=%s frame=%s img=%s): %s",
                                batch_idx, seq_idx, frame_idx, img_path, exc,
                            )
                            continue
                        except Exception as exc:
                            # 兜底：未预期的异常，记录后继续（不终止训练）
                            failed_steps += 1
                            self.logger.error(
                                "训练步未预期错误 (batch=%s seq=%s frame=%s): %s",
                                batch_idx, seq_idx, frame_idx, exc,
                            )
                            import traceback
            ...
            self.logger.info(
                "Epoch %d DataLoader 耗尽 (steps=%d frames=%d/%d), 开始收尾处理...",
                epoch + 1, num_steps, processed_frames, expected_frames,
            )
            ...
            if accum_step % self.config.gradient_accumulation != 0:
                try:
                    ...
                    self.logger.error("末尾梯度累积处理失败: %s", _acc_exc)
                    import traceback
                    self.logger.error("Traceback:\n%s", traceback.format_exc())
            ...
            current_lr = self.optimizer.param_groups[0]["lr"]
            nan_rate = nan_step_count / max(num_steps + nan_step_count, 1)

            self.logger.info(
                "Epoch %s 完成 | loss=%.6f cls=%.6f l1=%.6f ciou=%.6f reid=%.6f | P@50=%.4f R@50=%.4f MatchR=%.4f mIoU=%.4f | steps=%s failed=%s NaN=%s(%.1f%%) skipped_seq=%s | frames=%s/%s(%.3f) | %.2fs %.3f step/s %.3f frame/s",
                epoch + 1,
                avg_loss,
...
def train_three_stage(
    ...
    for stage_name in stage_names:
        ...
        if resume_info and resume_info.get("stage_name") == stage_name:
            ...
            print("\n" + "-" * 50)
            print(f"恢复阶段: {stage_name} (从 Epoch {resume_stage_epoch} 重跑)")
            print(f"已完成: {resume_stage_epoch} epochs, global_step={resume_global_step}")
            print("-" * 50 + "\n")

            # ─── Bug2修复：不再多减1 ───
            # 原代码 remaining_epochs = n_epochs - resume_stage_epoch - 1
            # 错误地将崩溃前未完成的 epoch 也丢弃了。
            # 正确逻辑：未完成的 epoch 必须完整重跑。
            remaining_epochs = n_epochs - resume_stage_epoch
            ...
            trainer._restore_optimizer_scheduler(
                resume_info,
                stage_cfg,
                n_epochs,           # Bug3修复：传递阶段总 epoch 数作为 T_max
                remaining_epochs,
            )
            ...
            trainer.train(
                ...
                # Bug2修复：不再 +1，从崩溃的 epoch 重新开始
                global_epoch_offset=stage_start_ep + resume_stage_epoch,
            ...
            # 创建新 optimizer（使用分层参数组，与 __init__ 保持一致，修复 Bug1：参数组扁平化）
            backbone_lr_scale = config.backbone_base_lr / config.head_base_lr
            param_groups = trainer.model.get_parameter_groups(
                base_lr=config.head_base_lr,
                backbone_lr_scale=backbone_lr_scale,
            )
            ...
            trainer.logger.info(
                "阶段优化器重置: %s, %s trainable params %s",
                stage_name, f"{n_trainable:,}", freeze_str,
            )
...
def create_combined_dataloader(
    ...
    # 创建各数据集实例（每份仅一个，不复制）
    ds_list = []
    ds_ratios = []  # 仅记录实际存在数据集的 ratio，避免索引错配
    for (ds_name, ds_path), ratio in zip(datasets_info, sample_ratio):
        if ratio <= 0:
            continue
    ...
    # 仅对实际存在的数据集按其 ratio 加权，归一化到总和为1
    weights = []
    total_ratio = sum(ds_ratios)
    for dataset, ratio in zip(ds_list, ds_ratios):
        w = ratio / total_ratio
    ...
    active_names = [d.dataset_name for d in ds_list]
    print(f"[CombinedData] 合并 {len(ds_list)} 个数据集, 共 {total_seqs} 序列, "
          f"采样权重 {active_names}={ds_ratios}")
